...
#[Route('/api/mobile/detailcommande', name: 'api_detailcommande')]
    ...
    #[Route('/api/mobile/actualites/{id}', name: 'api_mobile_get_actualite')]
public function getActualite(int $id, ActualiteRepository $actualiteRepository): JsonResponse
{
    $actualite = $actualiteRepository->find($id);

    if (!$actualite) {
        return new JsonResponse(['error' => 'Actualité non trouvée pour cet ID'], Response::HTTP_NOT_FOUND);
    }
    ...
    // Récupérer les images associées à l'actualité
    foreach ($actualite->getLesimages() as $image) {
        $response['images'][] = [
            'url' => $image->getUrl() // Supposons que vous avez une méthode getUrl() dans votre entité Image pour récupérer l'URL de l'image
            // Vous pouvez également ajouter d'autres détails de l'image ici si nécessaire
        ];
    }
...
#[Route('/api/mobile/getLesCategoriesParents', name: 'api_mobile_getLesCategories')]
public function getLesCategoriesParents(Request $request, EntityManagerInterface $entityManager, CategorieParentRepository $categorieParentRepository): Response
{
    // Supposant que $postdata est utilisé plus loin dans votre logique, sinon il peut être supprimé si non nécessaire.
    $postdata = json_decode($request->getContent());

    // Utilisez la nouvelle méthode du dépôt pour récupérer les catégories avec leurs parents.
    $var = $categorieParentRepository->findAllWithCategories(); 

    $response = new Utils;
    // Supposant que "lesProduits" et "laCategorieParent" sont des clés dont vous avez besoin dans votre réponse,
    // Ajustez en fonction de la structure réelle des données retournées par findAllWithCategories() si nécessaire.
    $tab = ["lescategories"];
